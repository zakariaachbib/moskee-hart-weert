    ...
    <>
      <section className="relative bg-brown overflow-hidden min-h-[350px] md:min-h-[400px]">
        ...
        <div className="container relative text-center py-24 md:py-32 flex flex-col items-center justify-end min-h-[350px] md:min-h-[400px]">
          ...
          <p className="text-cream/70 mt-4 max-w-xl mx-auto">{t.sermons.subtitle}</p>
        </div>
      </section>
      ...
      {viewingPdf && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 bg-black/90 flex flex-col items-center justify-center p-2 sm:p-4" onClick={() => setViewingPdf(null)}>
          <div className="w-full max-w-4xl flex justify-end mb-2">
            <button onClick={() => setViewingPdf(null)} className="text-white/70 hover:text-white"><X className="h-8 w-8" /></button>
          </div>
          <div className="w-full max-w-4xl flex-1 min-h-0" onClick={(e) => e.stopPropagation()}>
            <iframe src={viewingPdf} className="w-full h-full rounded-lg bg-white" style={{ minHeight: '70vh', maxHeight: '85vh' }} title="PDF Viewer" />
          </div>
        </motion.div>
      )}
    </>
