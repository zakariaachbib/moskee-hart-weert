        ...
        <div className="container max-w-5xl">
          ...
          {submitted ? (
            ...
            >
              ...
              <h3 className="font-heading text-2xl mb-2">{t.contact.sent}</h3>
              <p className="text-muted-foreground text-sm mb-2">{t.contact.sentDesc}</p>
              <p className="text-sm text-muted-foreground max-w-md mx-auto mb-6">
                Er is een bevestigingsmail verzonden naar <strong className="text-foreground">{form.email || "uw e-mailadres"}</strong>.
              </p>

              {/* WhatsApp button */}
              <a
                href={`https://wa.me/31616958298?text=${encodeURIComponent(
                  `Assalamu alaikum,\n\nIk heb zojuist een rondleiding aanvraag ingediend via de website.${selectedDate ? `\n\n📅 Gewenste datum: ${format(selectedDate, "d MMMM yyyy", { locale: nl })}` : ""}${selectedTime ? `\n🕐 Voorkeurstijd: ${selectedTime}` : ""}\n\nKunt u de beschikbaarheid bevestigen?\n\nBedankt!`
                )}`}
                target="_blank"
              ...
              <button
                onClick={() => { setSubmitted(false); setSelectedDate(undefined); setSelectedTime(""); }}
